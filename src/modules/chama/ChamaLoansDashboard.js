 ...
 *
 * SUB-MODULES:
 * "Applications", "Active Loans", "Repayments", and "Reports" tabs all
 * render the real, fully-wired modules (LoanApplications, LoanDisbursement,
 * LoanRepayments, LoanReports) — each of these owns its own Supabase
 * queries, its own loading/error state, and its own CSS, so this
 * dashboard only needs to load `loans` and `applications` for the
 * top-level KPI tiles on the Dashboard tab.
 *
 * NOTE ON "Active Loans": LoanDisbursement shows the queue of loans that
 * are approved but not yet disbursed (chama_loans.disbursed = false), not
 * the full active loan book — so it's rendered above this file's own
 * loan table (all loans, sortable/filterable by status) rather than
 * replacing it. If LoanApplications.finalizeLoan doesn't explicitly set
 * disbursed: false when a loan is created, newly-approved loans may not
 * appear in the disbursement queue unless your chama_loans table has a
 * matching column default in Supabase.
 */
...
export default function ChamaLoansDashboard({ chamaId }) {
  ...
  // Only `loans` and `applications` are needed here now — Repayments and
  // Reports each fetch their own data independently (see their files).
  const loadData = useCallback(async () => {
    setLoading(true);
    setError(null);

    try {
      const [loansRes, applicationsRes] = await Promise.all([
  ...
  return (
    <div className="cld-root">
      ...
      <div className="cld-content">
        ...
        {activeTab === "applications" && <LoanApplications chamaId={chamaId} />}

        {activeTab === "loans" && (
          <>
            <LoanDisbursement chamaId={chamaId} />

            <div className="cld-panel">
              <div className="cld-panel-title-row">
                <div className="cld-panel-title">Full Loan Book</div>
        ...
        {activeTab === "repayments" && <LoanRepayments chamaId={chamaId} />}

        {activeTab === "reports" && <LoanReports chamaId={chamaId} />}
      </div>
    </div>
  );
}
