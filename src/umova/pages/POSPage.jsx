...
import { useProducts } from '../../part1/hooks/useProducts';
...
export default function POSPage({ businessId, branchId }) {
  const { user } = useContext(AuthContext);
  const tenantId = user?.tenant_id;
  const { products, fetch: refetchProducts } = useProducts(businessId);
  const { create: createSale } = useSales(businessId, branchId);
  const { activeShift, openShift, closeShift } = useCashierShifts(businessId, branchId);
  ...
  return (
    <div className="h-screen flex flex-col">
      <div className="bg-gray-800 text-white p-3 flex justify-between items-center">
        ...
        <button onClick={() => { if (confirm('Close shift?')) closeShift(0, ''); }} className="bg-red-600 px-3 py-1 rounded text-sm">Close Shift</button>
      </div>
