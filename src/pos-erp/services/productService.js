...
//
// Adapted from the original POS module: tenant_id/business_id stripped
// throughout (this app is single-org — see usePosErpAuth.js for why).
// Everything else (table names, select shape, error handling) kept as-is.

import { supabase } from '../../supabaseClient';

export const productService = {
  async getAll({ categoryId, search, page = 1, limit = 50 } = {}) {
    let q = supabase
      .from('lb_products')
      .select('*, category:lb_product_categories(id,name), unit:lb_product_units(id,name,abbreviation)', { count: 'exact' })
      .order('name');
  ...
  async getById(id) {
    const { data, error } = await supabase
      .from('lb_products')
      .select('*, category:lb_product_categories(id,name), unit:lb_product_units(id,name,abbreviation)')
      .eq('id', id)
      .single();
