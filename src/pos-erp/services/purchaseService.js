...
// is single-org, single-location — see usePosErpAuth.js for why). There
// is no lb_branches table either, so the branch: lb_branches(...) joins
// and branch-scoped filters from the original module are dropped along
// with it. Table names, item shapes, and the GRN -> inventory -> PO
// rollup -> supplier invoice pipeline are otherwise kept as-is.
...
export const purchaseOrderService = {
  async getAll({ status, supplierId, page = 1, limit = 50 } = {}) {
    let query = supabase
      .from(PO_TABLE)
      .select(`*, supplier:lb_suppliers(id, name), items:${POI_TABLE}(*, product:lb_products(id, name, sku))`, { count: 'exact' })
  ...
  async getById(id) {
    const { data, error } = await supabase
      .from(PO_TABLE)
      .select(`*, supplier:lb_suppliers(id, name, phone), items:${POI_TABLE}(*, product:lb_products(id, name, sku, unit_id), unit:lb_product_units(id, name, abbreviation))`)
      .eq('id', id)
      .single();
    if (error) throw error;
    return data;
...
export const goodsReceivedService = {
  async getAll({ status, page = 1, limit = 50 } = {}) {
    let query = supabase
      .from(GRN_TABLE)
      .select(`*, supplier:lb_suppliers(id, name), purchase_order:lb_purchase_orders(id, order_number), items:${GRI_TABLE}(*, product:lb_products(id, name, sku))`, { count: 'exact' })
  ...
  async getById(id) {
    const { data, error } = await supabase
      .from(GRN_TABLE)
      .select(`*, supplier:lb_suppliers(id, name, phone), purchase_order:lb_purchase_orders(id, order_number), received_by_user:users!lb_goods_received_received_by_fkey(id, full_name), items:${GRI_TABLE}(*, product:lb_products(id, name, sku, unit_id), unit:lb_product_units(id, name, abbreviation))`)
      .eq('id', id)
      .single();
    if (error) throw error;
    return data;
...
async function updateInventoryOnReceipt({ productId, quantity, unitCost, batchNumber, expiryDate, referenceId, referenceType }) {
...
async function updateInventoryOnAdjustment({ productId, quantity, reason, referenceId, referenceType }) {
  const { data: existing } = await supabase
    .from('lb_inventory')
    .select('*')
    .eq('product_id', productId)
